...
TEST_CASE(QuanpinDictionaryOnlyTriesMultipleSegmentationsForAtMostFourSyllables)
{
    const auto db_path = CreatePinyinCacheDatabase();
    {
        QuanpinDictionary dictionary(db_path.string());
        ...
        // MSIME-Engine#5 raised kMaxSyllablesForMultipleSegmentations from 3 to 4 so
        // that the dictionary, not the tie-break rule, decides cases like jianmingeyao.
        ...
        // The limit still exists; five syllables stay on the single segmentation.
        const auto five_syllable_candidates =
            dictionary.query("xianxianxianxianxian", "xian'xian'xian'xian'xian");
